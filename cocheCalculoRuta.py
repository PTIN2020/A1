...
try:
    ...
    while True: #emulació d'un do...while
        ...
        print("hola")
        if peticio.text != "No hay más pasajeros":
            item = peticio.json();
            print(item)
            ...
            if str(item) != "[]":
                ...
                while f == 1:
                    print("coche")
                    peticioCotxe = requests.get('http://craaxcloud.epsevg.upc.edu:36302/coches/disponibles')
                    while peticioCotxe.status_code != 200:
                        peticioCotxe = requests.get('http://craaxcloud.epsevg.upc.edu:36302/coches/disponibles')
                    listItemCotxe = peticioCotxe.json()
                    print("Coche disponible")
                    if str(listItemCotxe) != "[]":
                        f=0
                itemCotxe = listItemCotxe[0]
                numCotxe = itemCotxe['id_coche']
                #print(cotxe['puntOrigencotxe'])
                puntOrigencotxe = itemCotxe['puntOrigencotxe']
                ...
                #url = 'http://192.168.1.71:3000/coches/'+str(numCotxe)
                url = 'http://craaxcloud.epsevg.upc.edu:36302/coches/'+str(numCotxe)
                print("Cojo pasajero")
                strcami = ""
                strcamicotxe = ""
                for i, elem in enumerate(cami):
                    strcami = strcami + elem + ", "
                
                for i, elem in enumerate(camicotxe):
                    strcamicotxe = strcamicotxe + elem + ", "
                
               
                actualitzaCotxe = {'puntOrigen':puntOrigenPasajero, 'puntDesti': puntDesti, 'ruta_pasajero': strcamicotxe, 'ruta_desti': strcami, 'estado': 'buscando', 'id_pasajero': pasajero}
                print("en curs...")
                
                x = requests.put(url, json = actualitzaCotxe)
